package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      private var var_1348:Rectangle;
      ...
      private var var_285:Rectangle;
      
      private var var_1349:Point;
      ...
      public function WindowRenderer(param1:ISkinContainer)
      {
         ...
         var_518 = new Array();
         var_1349 = new Point();
         var_1348 = new Rectangle();
         var_285 = new Rectangle();
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_518.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_285.x = _loc3_.dirty.x;
               var_285.y = _loc3_.dirty.y;
               var_285.width = _loc3_.dirty.width;
               var_285.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_285);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_608,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_286[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1349,var_1348))
            {
               if(param2.render(param1,var_1349,var_1348))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_615);
            var_286[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_608,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_285.x = param2.x - param1.x;
            var_285.y = param2.y - param1.y;
            var_285.width = param2.width;
            var_285.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_32))
            {
               while(param1.testParamFlag(WindowParam.const_32))
               {
                  ...
                  var_285.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_285,WindowRedrawFlag.const_265);
            if(var_518.indexOf(param1) == -1)
            {
               var_518.push(param1);
            }
         }
