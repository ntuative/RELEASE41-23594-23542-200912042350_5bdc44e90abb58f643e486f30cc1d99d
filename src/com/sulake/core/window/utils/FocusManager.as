package com.sulake.core.window.utils
{
   ...
   public class FocusManager extends ChildEntityArrayReader implements IWindowFocusManagerService
   {
      ...
      {
         var_189 = Stage(param1);
         var_189.addEventListener(Event.ACTIVATE,onActivateEvent);
         var_189.addEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
         var_189.addEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
         var_189.addEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
         super();
      }
      ...
      public function dispose() : void
      {
         if(!_disposed)
         {
            var_189.removeEventListener(Event.ACTIVATE,onActivateEvent);
            var_189.removeEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
            var_189.removeEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
            var_189.removeEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
            var_189 = null;
            _disposed = true;
            var_39 = null;
         }
