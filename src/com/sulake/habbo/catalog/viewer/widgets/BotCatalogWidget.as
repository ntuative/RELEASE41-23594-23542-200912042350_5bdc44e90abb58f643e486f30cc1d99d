package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class BotCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid
   {
      ...
      protected var var_1299:XML;
      
      private var var_1386:Map;
      
      private var var_764:Offer;
      
      public function BotCatalogWidget(param1:IWindowContainer)
      {
         super(param1);
         var_871 = _window.findChildByName("botGrid") as IItemGridWindow;
      }
      ...
      private function populateGridItems() : void
      {
         ...
         var_1299 = _loc1_.content as XML;
         var _loc2_:int = 0;
      ...
      {
         var _loc2_:IWindowContainer = page.viewer.catalog.windowManager.buildFromXML(var_1299) as IWindowContainer;
         var_871.addGridItem(_loc2_);
         param1.productContainer.view = _loc2_;
         var _loc3_:String = var_1386.getValue(param1.productContainer.firstProduct.extraParam.toLowerCase());
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createAvatarImage(_loc3_,AvatarScaleType.const_102);
      ...
      {
         var _loc1_:String = var_1386.getValue(var_764.productContainer.firstProduct.extraParam.toLowerCase());
      ...
      {
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_764,page);
      }
      ...
      private function setProductData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc1_.registerParameter("catalog.purchase.price.credits+pixels","credits",String(var_764.priceInCredits));
            _loc1_.registerParameter("catalog.purchase.price.credits+pixels","pixels",String(var_764.priceInPixels));
            _loc2_.text = "${catalog.purchase.price.credits+pixels}";
         }
         var _loc3_:IProductData = page.viewer.catalog.getProductData(var_764.localizationId);
